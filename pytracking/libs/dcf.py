...
from pytracking import complex, fourier
...
def hann1d(sz: int, centered=True) -> torch.Tensor:
    ...
    w = 0.5 * (1 + torch.cos((2 * math.pi / (sz + 2))
                             * torch.arange(0, sz//2 + 1).float()))
...
def hann2d_clipped(sz: torch.Tensor, effective_sz: torch.Tensor, centered=True) -> torch.Tensor:
    ...
    effective_window = hann1d(effective_sz[0].item(), True).reshape(
        1, 1, -1, 1) * hann1d(effective_sz[1].item(), True).reshape(1, 1, 1, -1)

    pad = (sz - effective_sz) / 2

    window = F.pad(effective_window, (pad[1].item(
    ), pad[1].item(), pad[0].item(), pad[0].item()), 'replicate')
...
def label_function_spatial(sz: torch.Tensor, sigma: torch.Tensor, center: torch.Tensor = torch.zeros(2), end_pad: torch.Tensor = torch.zeros(2)):
    ...
    return gauss_spatial(sz[0].item(), sigma[0].item(), center[0], end_pad[0].item()).reshape(1, 1, -1, 1) * \
        gauss_spatial(sz[1].item(), sigma[1].item(), center[1],
                      end_pad[1].item()).reshape(1, 1, 1, -1)
...
def get_interp_fourier(sz: torch.Tensor, method='ideal', bicubic_param=0.5, centering=True, windowing=False, device='cpu'):
    ...
    if centering:
        interp_y = complex.mult(
            interp_y, complex.exp_imag((-math.pi/sz[0]) * ky))
        interp_x = complex.mult(
            interp_x, complex.exp_imag((-math.pi/sz[1]) * kx))
...
def get_reg_filter(sz: torch.Tensor, target_sz: torch.Tensor, params):
    ...
    if getattr(params, 'reg_window_centered', True):
        wrg = torch.arange(-int((sz[0]-1)/2), int(sz[0]/2+1),
                           dtype=torch.float32).view(1, 1, -1, 1)
        wcg = torch.arange(-int((sz[1]-1)/2), int(sz[1]/2+1),
                           dtype=torch.float32).view(1, 1, 1, -1)
    else:
    ...
    reg_window_dft[reg_window_dft_abs <
                   params.reg_sparsity_threshold * reg_window_dft_abs.max(), :] = 0

    # Do the inverse transform to correct for the window minimum
    reg_window_sparse = torch.irfft(
        reg_window_dft, 2, signal_sizes=sz.long().tolist())
    reg_window_dft[0, 0, 0, 0, 0] += params.reg_window_min - \
        sz.prod() * reg_window_sparse.min()
...
def max2d(a: torch.Tensor) -> (torch.Tensor, torch.Tensor):
    ...
    argmax_row = argmax_row.view(
        argmax_col.numel(), -1)[torch.arange(argmax_col.numel()), argmax_col.view(-1)]
    argmax_row = argmax_row.reshape(argmax_col.shape)
    argmax = torch.cat(
        (argmax_row.unsqueeze(-1), argmax_col.unsqueeze(-1)), -1)
