...
from pytracking import TensorList, dcf
from pytracking.features import augmentation
from pytracking.features.preprocessing import (numpy_to_torch,
                                               sample_patch_multiscale,
                                               sample_patch_transformed)
from pytracking.tracker.base import BaseTracker
from pytracking.utils.plotting import plot_graph, show_tensor
...
class DiMP(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if update_flag and self.params.get('update_classifier', False):
            ...
            self.update_classifier(train_x, target_box,
    ...
    def get_sample_location(self, sample_coord):
        ...
        sample_scales = (
            (sample_coord[:, 2:] - sample_coord[:, :2]) / self.img_sample_sz).prod(dim=1).sqrt()
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        aug_expansion_factor = self.params.get(
        ...
        if random_shift_factor > 0:
            def get_rand_shift(): return ((torch.rand(2) - 0.5) * self.img_sample_sz *
        ...
        self.transforms = [augmentation.Identity(
            aug_output_sz, global_shift.long().tolist())]

        augs = self.params.augmentation if self.params.get(
    ...
    def init_target_boxes(self):
        ...
        for T in self.transforms:
            init_target_boxes.append(
                self.classifier_target_box + torch.Tensor([T.shift[1], T.shift[0], 0, 0]))
        init_target_boxes = torch.cat(
            init_target_boxes.view(1, 4), 0).to(self.params.device)
    ...
    def update_sample_weights(self, sample_weights, previous_replace_ind, num_stored_samples, num_init_samples, learning_rate=None):
        ...
        for sw, prev_ind, num_samp, num_init in zip(sample_weights, previous_replace_ind, num_stored_samples, num_init_samples):
            ...
            init_samp_weight = self.params.get(
    ...
    def init_iou_net(self, backbone_feat):
        ...
        target_boxes = torch.cat(target_boxes.view(
            1, 4), 0).to(self.params.device)
        ...
        self.iou_modulation = self.get_iou_modulation(
            iou_backbone_feat, target_boxes)
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        if 'dropout' in self.params.augmentation and self.params.get('use_augmentation', True):
            ...
            x = torch.cat([x, F.dropout2d(x[0:1, ...].expand(
                num, -1, -1, -1), p=prob, training=True)])
        ...
        if self.params.get('window_output', False):
            if self.params.get('use_clipped_window', False):
                ...
                ), (self.output_sz*self.params.effective_search_area / self.params.search_area_scale).long(), centered=True).to(self.params.device)
            else:
                self.output_window = dcf.hann2d(
                    self.output_sz.long(), centered=True).to(self.params.device)
        ...
        if plot_loss:
            ...
            if self.visdom is not None:
                self.visdom.register((self.losses, torch.arange(
                    self.losses.numel())), 'lineplot', 3, 'Training Loss' + self.id_str)
            elif self.params.debug >= 3:
                plot_graph(self.losses, 10,
                           title='Training Loss' + self.id_str)

    def _overwrite_classifier_params(self, feature_dim):
        # Overwrite some parameters in the classifier. (These are not generally changed)
        pred_module = getattr(self.net.classifier.filter_optimizer,
                              'score_predictor', self.net.classifier.filter_optimizer)
        ...
        if self.params.get('filter_init_zero', False):
            self.net.classifier.filter_initializer = FilterInitializerZero(
                self.net.classifier.filter_size, feature_dim)
    ...
    def update_classifier(self, train_x, target_box, learning_rate=None, scores=None):
        ...
        if hard_negative_flag or self.frame_num % self.params.get('train_sample_interval', 1) == 0:
            self.update_memory(TensorList(
                [train_x]), target_box, learning_rate)
        ...
        if num_iter > 0:
            ...
            samples = self.training_samples[0][:
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale=True):
        ...
        init_box = self.get_iounet_box(
            self.pos, self.target_sz, sample_pos, sample_scale)
        ...
        if self.params.num_init_random_boxes > 0:
            ...
            rand_factor = square_box_sz * \
                torch.cat([self.params.box_jitter_pos * torch.ones(2),
                           self.params.box_jitter_sz * torch.ones(2)])

            minimal_edge_size = init_box[2:].min()/3
            rand_bb = (torch.rand(
                self.params.num_init_random_boxes, 4) - 0.5) * rand_factor
        ...
        # Optimize the boxes
        output_boxes, output_iou = self.optimize_boxes(
            iou_features, init_boxes)
        ...
        new_pos = (new_pos.flip((0,)) - (self.img_sample_sz - 1) /
                   2) * sample_scale + sample_pos
    ...
    def optimize_boxes(self, iou_features, init_boxes):
        box_refinement_space = self.params.get(
    ...
    def optimize_boxes_default(self, iou_features, init_boxes):
        ...
        if isinstance(step_length, (tuple, list)):
            step_length = torch.Tensor(
                [step_length[0], step_length[0], step_length[1], step_length[1]], device=self.params.device).view(1, 1, 4)
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            # Update proposal
            output_boxes = bb_init + step_length * \
                bb_init.grad * bb_init[:, :, 2:].repeat(1, 1, 2)
    ...
    def direct_box_regression(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale=True):
        ...
        # Optimize the boxes
        output_boxes = self.net.bb_regressor.predict_bb(
            self.iou_modulation, iou_features, init_boxes).view(-1, 4).cpu()
        ...
        new_pos = (new_pos.flip((0,)) - (self.img_sample_sz - 1) /
                   2) * sample_scale + sample_pos
        new_target_sz = predicted_box[2:].flip((0,)) * sample_scale
        new_scale_bbr = torch.sqrt(
            new_target_sz.prod() / self.base_target_sz.prod())
    ...
    def visualize_iou_pred(self, iou_features, center_box):
        ...
        pos_boxes = bbutils.rel_to_rect(
            (center_box_rel + bbx) + bby, sz_norm).view(1, -1, 4).to(self.params.device)
        sz_boxes = bbutils.rel_to_rect(
            (center_box_rel + bbw) + bbh, sz_norm).view(1, -1, 4).to(self.params.device)

        pos_scores = self.net.bb_regressor.predict_iou(
            self.iou_modulation, iou_features, pos_boxes).exp()
        sz_scores = self.net.bb_regressor.predict_iou(
            self.iou_modulation, iou_features, sz_boxes).exp()

        show_tensor(pos_scores.view(pos_scale.numel(), -1),
                    title='Position scores', fig_num=21)
        show_tensor(sz_scores.view(sz_scale.numel(), -1),
                    title='Size scores', fig_num=22)

    def visdom_draw_tracking(self, image, box, segmentation=None):
        if hasattr(self, 'search_area_box'):
            self.visdom.register(
                (image, box, self.search_area_box), 'Tracking', 1, 'Tracking')
        else:
